...
var Class=O.Class=function(nam,specs,onReady){
   ...
   ,  SetupVars: function(obj, priv, pub, thisClass) {
         var z, z2, z4, zz, k, k2, k4, stt, s2, ya, vd
               tc=thisClass
            ,  xt=(tc!==obj)?
                     {
                           T: obj
         ...
         for(nm in zz) {
            ...
            if(typeof z=='function' && !z.$$sys && !z.$$bound) {
               if(z.$isOverload) {
                  ...
                  var gt=k4.Get, st=k4.Set, os;
                  ya=0;
                  
                  if(gt && !gt.$$sys && !gt.$$bound) gt=s2.EvalLib(gt);
                  if(st && !st.$$sys && !st.$$bound) st=s2.EvalLib(st);
                  
                  os={ get: gt };
                  if(!k4.readonly || k4.Set) {
                     os.set=st;
                     os.writable=true;
                  };
